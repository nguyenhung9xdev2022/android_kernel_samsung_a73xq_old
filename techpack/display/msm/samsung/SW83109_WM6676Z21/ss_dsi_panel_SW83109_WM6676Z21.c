...
*/
#include "ss_dsi_panel_SW83109_WM6676Z21.h"
#include "ss_dsi_mdnie_SW83109_WM6676Z21.h"
...
static int ss_module_info_read(struct samsung_display_driver_data *vdd)
{
	...
	if (ss_get_cmds(vdd, RX_MODULE_INFO)->count) {
		...
		vdd->cell_id_dsi[11] = buf[11];
		vdd->cell_id_dsi[12] = buf[12];

		LCD_INFO(vdd, "DSI%d CELL ID : %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
...
static void ss_update_panel_lpm_ctrl_cmd(struct samsung_display_driver_data *vdd, int enable)
{
	...
	LCD_INFO(vdd, "[SW83109_WM6676Z21_CSOT] ++\n");
...
start_lpm_bl:
	...
	case LPM_60NIT:
		...
		set_lpm_off->cmds[1].ss_txbuf[2] = 0xE2;
		break;
	case LPM_30NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_30NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x03;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0x84;
		break;
	case LPM_10NIT:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_10NIT_CMD);
		set_lpm_off->cmds[1].ss_txbuf[1] = 0x01;
		set_lpm_off->cmds[1].ss_txbuf[2] = 0xFF;
		break;
	case LPM_2NIT:
	default:
		set_lpm_bl = ss_get_cmds(vdd, TX_LPM_2NIT_CMD);
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	make_self_dispaly_img_cmds_SW83109_WM6676Z21(vdd, TX_SELF_MASK_IMAGE, FLAG_SELF_MASK);
	...
	make_self_dispaly_img_cmds_SW83109_WM6676Z21(vdd, TX_SELF_MASK_IMAGE_CRC, FLAG_SELF_MASK_CRC);
...
void SW83109_WM6676Z21_FHD_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.init = self_display_init_SW83109_WM6676Z21;

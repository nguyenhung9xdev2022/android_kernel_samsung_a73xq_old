...
static int sec_direct_chg_check_charging_source(struct sec_direct_charger_info *charger)
{
	...
	charger->voltage_pack_main = value.intval;

	/* get sub pack voltage */
	value.intval = SEC_DUAL_BATTERY_SUB;
	psy_do_property("sec-dual-battery", get,
		POWER_SUPPLY_PROP_VOLTAGE_AVG, value);
	charger->voltage_pack_sub = value.intval;
#endif
	...
	if (charger->direct_chg_done || (charger->capacity >= 95) || !value.intval || charger->store_mode ||
		(charger->voltage_pack_main >= charger->pdata->sc_vbat_thresh) || (charger->voltage_pack_sub >= charger->pdata->sc_vbat_thresh))
#else
	if (charger->direct_chg_done || (charger->capacity >= 95) || !value.intval || charger->store_mode)
#endif
	{
		...
		unsigned int charger_mode, int charging_source)
{
	...
	if (charging_source == SEC_CHARGING_SOURCE_DIRECT &&
		(charger_mode == SEC_BAT_CHG_MODE_CHARGING || charger_mode == SEC_BAT_CHG_MODE_PASS_THROUGH)) {
...
#if IS_ENABLED(CONFIG_DUAL_BATTERY)
		ret = of_property_read_u32(np, "charger,sc_vbat_thresh",
			&charger->pdata->sc_vbat_thresh);
		if (ret) {
			pr_err("%s : charger,sc_vbat_thresh is Empty\n", __func__);
			charger->pdata->sc_vbat_thresh = 4420;
		}
		pr_info("%s: charger,sc_vbat_thresh is %d\n", __func__, charger->pdata->sc_vbat_thresh);
#endif
